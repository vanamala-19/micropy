from ST7735 import TFT
from sysfont import sysfont
from machine import SPI,Pin
import convert as c
from Keypad import K
import utime
import math

tn = sysfont["Height"]
n =sysfont["Height"]*18
R = 0.287
CP = 1.005;
CV = 0.716

spi = SPI(1, baudrate=20000000, polarity=0, phase=0,
          sck=Pin(10), mosi=Pin(11), miso=None)
tft=TFT(spi,16,17,18)
tft.initr()
tft.rgb(True)

k = K()


class CFC(object) :

    def __init__( self) :
       self.ma=0
       self.mau=0
       self.d=0
       self.du=0
       self.p1=0
       self.p1u=0
       self.p2=0
       self.p2u=0
       self.t1=0
       self.t1u=0
       self.t2=0
       self.t2u=0
       self.T=0
       self.Pin=0
       self.Win=0
       self.isene=0
       self.Ds=0
       self.img = str()
       self.graph1 = ["Win","Pow","Ds"]
       self.graph2 = []
       self.graph = "power"
    
    def stage1(self):
        tft.fill(TFT.BLACK )
        c=0
        d=0
        tft.text((c, d), "Enter the Values: ", TFT.GREEN, sysfont, 1)
        utime.sleep(0.3)
        d += 1.5*tn;
        tft.text((c, d), "1)Mass flow rate(ma):", TFT.GREEN, sysfont, 1)
        self.ma = k.Read(tn*16,d-tn)
        utime.sleep(0.3)
        d += 1.5*tn;
        tft.text((c, d), "Units", TFT.GREEN, sysfont, 1)
        tft.text((tn*3, d+tn), "1)Kg/s  2)Kg/min", TFT.GREEN, sysfont, 1)
        self.mau = k.Read(tn*16,d)
        utime.sleep(0.3)
        d += 2.6*tn;
        tft.text((c, d), "2) Pressure p1 :", TFT.GREEN, sysfont, 1)
        self.p1 = k.Read(tn*16,d)
        utime.sleep(0.3)
        d += 1.5*tn;
        tft.text((c, d), "Units:", TFT.GREEN, sysfont, 1)
        tft.text((2*tn, d+tn), "1)Bars   2)KN/m^2   3)Pascls", TFT.GREEN, sysfont, 1)
        self.p1u = k.Read(tn*16,d)
        utime.sleep(0.3)
        d +=3.4*tn
        tft.text((c, d), "3) Pressure p2 :", TFT.GREEN, sysfont, 1)
        self.p2 = k.Read(tn*16,d)
        utime.sleep(0.3)
        d +=1.5*tn
        tft.text((c, d), "Units:", TFT.GREEN, sysfont, 1)
        tft.text((2*tn, d+tn), "1)Bars   2)KN/m^2   3)Pascls", TFT.GREEN, sysfont, 1)
        self.p2u = k.Read(tn*16,d)
        utime.sleep(0.3)
        
    def stage2(self):
        tft.fill(TFT.BLACK )
        c=0
        d=0
        tft.text((c, d), "4)Tempurature T1:", TFT.GREEN, sysfont, 1)
        self.t1 = k.Read(tn*14,d-tn)
        utime.sleep(0.3)
        d += 1.8*tn;
        tft.text((c, d), "Units", TFT.GREEN, sysfont, 1)
        tft.text((tn*3, d+tn), "1)celsius  2)Kelvin 3)fahrenheit", TFT.GREEN, sysfont, 1)
        self.t1u = k.Read(tn*16,d)
        utime.sleep(0.3)
        d += 3.5*tn;
        tft.text((c, d), "5)Tempurature T2:", TFT.GREEN, sysfont, 1)
        self.t2 = k.Read(tn*14,d-tn)
        utime.sleep(0.3)
        d += 1.8*tn;
        tft.text((c, d), "Units", TFT.GREEN, sysfont, 1)
        tft.text((tn*3, d+tn), "1)celsius  2)Kelvin 3)fahrenheit", TFT.GREEN, sysfont, 1)
        self.t2u = k.Read(tn*16,d)
        utime.sleep(0.3)
        
    def calculate(self):
        try:
            self.p1 = c.diameter(self.p1,(self.p1u))
            self.p2 = c.diameter(self.p2,(self.p2u))
            self.t2 = c.tempurature(self.t2,(self.t2u))
            self.t1 = c.tempurature(self.t1,self.t1u)
            self.ma = c.mass(self.ma,self.mau)               
            n = 1.4;
            T2x = (self.t1 * math.pow((self.p2 / self.p1), (n - 1) / n))
            self.isene = (((T2x - self.t1) / (self.t2 - self.t1)) * 100)
            self.Pin = (self.ma *CP * (self.t2 - self.t1))
            self.Win = (CP * (self.t2 - self.t1))
            self.Ds = (self.ma * (CP * math.log(self.t2 / self.t1) - R * math.log(self.p2 / self.p1)))
            self.graph2 = [self.Win,self.Pin,self.Ds]
            return True
        except: 
            tft.text((3*tn, 2*tn),"INCORRECT ", TFT.GREEN, sysfont, 2)
            tft.text((3*tn, 6*tn),"  INPUT ", TFT.GREEN, sysfont, 2)
            tft.text((3*tn, 9*tn),"TRY AGAIN", TFT.GREEN, sysfont, 2)
            return False
    
    def table(self):
        c=tn
        d=0
        tft.text((c, d), "1)Pin :", TFT.GREEN, sysfont, 1)
        tft.text((tn*9,0 ),str(round(self.Pin,2)), TFT.GREEN, sysfont, 1)
        tft.text((tn*15,0 ),"KW", TFT.GREEN, sysfont, 1,nowrap=True)
        utime.sleep(0.2)
        d += 1.8*tn;
        tft.text((c, d), "2) Win:", TFT.GREEN, sysfont, 1)
        tft.text((tn*9,d),str(round(self.Win,2)), TFT.GREEN, sysfont, 1)
        tft.text((tn*15,d ),"KW", TFT.GREEN, sysfont, 1,nowrap=True)
        utime.sleep(0.2)
        d += 1.5*tn;
        tft.text((c, d), "3) Ds:", TFT.GREEN, sysfont, 1)
        tft.text((tn*9,d),str(round(self.Ds,2)), TFT.GREEN, sysfont, 1)
        tft.text((tn*15,d),"KW", TFT.GREEN, sysfont, 1,nowrap=True)
        utime.sleep(0.2)
        d += 1.8*tn;
        tft.text((c, d), "4)isene:", TFT.GREEN, sysfont, 1)
        tft.text((tn*9,d),str(round(self.isene,2)), TFT.GREEN, sysfont, 1)
        tft.text((tn*15,d),"%", TFT.GREEN, sysfont, 1,nowrap=True)
        utime.sleep(0.2)
    

    

        